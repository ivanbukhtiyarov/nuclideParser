...
int main()
{
    ...
    */
    ...
    DecayMatrix dm(mychain.name_idx.size());
    dm.form_matrixreal(mychain);
    //IterMatrix im(dm);
    //auto matrix = im.matrixdev(mychain, *materials[0]);
    //auto sigp = im.dsigp(mychain, *materials[0]);
    CramMatrix cm(dm);
    auto matrix = cm.matrixreal(mychain, *materials[0]);
    auto newconc = make_concentration(mychain, materials[1]->namenuclides, materials[1]->conc);*/
